...
// Vercel crée plusieurs variables pour une même base Upstash. On veut spécifiquement
// la paire "REST" (URL en https://), pas la paire "rediss://" (protocole natif Redis,
// incompatible avec @upstash/redis). On regarde donc KV_REST_API_* EN PREMIER.
//
// IMPORTANT : on ne crée la connexion qu'à la première utilisation réelle (lazy),
// jamais au chargement du fichier, sinon le build Vercel plante.
let _redis = null;
function getRedis() {
  ...
  if (!url || !token) {
    throw new Error(
      "Base de données non configurée : ajoute KV_REST_API_URL/KV_REST_API_TOKEN dans les variables d'environnement Vercel, puis redéploie."
    );
  }
  if (!url.startsWith("https://")) {
    throw new Error(
      `URL Redis invalide (doit commencer par https://). Utilise KV_REST_API_URL, pas REDIS_URL. Reçu : "${url}"`
    );
  }
