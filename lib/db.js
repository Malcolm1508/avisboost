...
// la paire "REST" (URL en https://). On regarde KV_REST_API_* EN PREMIER.
let _redis = null;
function getRedis() {
  ...
  if (!url || !token) {
    throw new Error("Base de données non configurée (KV_REST_API_URL / KV_REST_API_TOKEN).");
  }
  if (!url.startsWith("https://")) {
    throw new Error(`URL Redis invalide (doit commencer par https://). Utilise KV_REST_API_URL. Reçu : "${url}"`);
  }
...
// --- SUIVI DES AVIS (feature 2 : conversion scan -> avis) ---
export async function getReviews(id) {
  ...
  return {
    ...
    tapsAtBase: Number(data.tapsAtBase || 0),
