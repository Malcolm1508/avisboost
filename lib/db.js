...
// Les données de CHAQUE client commerçant sont rangées par son identifiant (id).
...
export async function saveClient(id, data) {
  await redis.hset(`client:${id}`, data);
  await redis.sadd("clients", id); // on garde la liste de tous les ids
}
...
export async function incrTap(id) {
  ...
  await redis.incr(`taps:${id}`);        // total
  await redis.incr(`taps:${id}:${day}`); // par jour
}

export async function getTaps(id) {
  const total = await redis.get(`taps:${id}`);
